 ...
 */
/*
  Repository path: $HeadURL: https://svn.nq.pl/wordpress/branches/dynamite/igniter/wp-content/themes/igniter/lib/SilverWp/Sidebar/Widget/WidgetAbstract.php $
  Last committed: $Revision: 2184 $
  Last changed by: $Author: padalec $
  Last changed date: $Date: 2015-01-21 13:20:08 +0100 (Śr, 21 sty 2015) $
  ID: $Id: WidgetAbstract.php 2184 2015-01-21 12:20:08Z padalec $
 */
...
require_once LIBS_PATH. 'WidgetsHelperClass/wph-widget-class.php';
/**
...
abstract class WidgetAbstract extends WPH_Widget
{
    ...
    public function create_field_category_cb($key)
    {
        ...
        //$output .= '</p>';
        $output .= $this->create_field_label($key['name'], $key['_id']) . '<br/>';
        $categories = get_categories('hide_empty=0');

        $option = '';
        foreach ($categories as $cat) {
            $option .= '<label><input type="checkbox" id="'. esc_attr($key['_id']) .'" ';
            $option .= 'name="'. esc_attr($key['_name']).'[]"';
            if (isset($key['value']) && is_array($key['value']) && count($key['value'])) {
