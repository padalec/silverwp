	...
	abstract class PostTypeAbstract extends SingletonAbstract implements PostTypeInterface, Core {
		...
		public function addCounts() {
			...
			if ( $num_posts->pending > 0 ) {
				...
				$text = Translate::n( 'Item Pending', 'Items Pending',
					\intval( $num_posts->pending ) );
				if ( \current_user_can( 'edit_posts' ) ) {
					$num
						= "<a href=\"edit.php?post_status=pending&post_type={$this->name}\">$num</a>";
					$text
						= "<a href=\"edit.php?post_status=pending&post_type={$this->name}\">$text</a>";
				}
				echo '<td class="first b b-portfolio">' . $num . '</td>';
				echo '<td class="t portfolio">' . $text . '</td>';
				echo '</tr>';
		...
		public function getQueryData(
			$limit = 5, $paginator = false, array $query_args = array()
		) {
			$return    = array();
			$like_bool = Option::get_theme_option( $this->getName()
			                                       . '_list_like' );
			if ( $like_bool === '1' ) {
			...
			if ( $loop->have_posts() ) {
				...
				while ( $loop->have_posts() ) {
					...
					$like_count = $like_bool === '1'
						? $PostLike->getPostLikeCount( $post_id ) : '';
					//add meta box
					if ( $this->isMetaBoxRegistered() ) {
						$meta_box_all = $this->getMetaBox()
						                     ->setPostId( $post_id )->getAll();
						//Fix Ticket #319
						if ( ! empty( $meta_box_all ) ) {
							$mata_box
								= RecursiveArray::removeEmpty( $meta_box_all );
						} else {
							$mata_box = array();
						}
					} else {
					...
					if ( $this->isTaxonomyRegistered()
					     && $this->getTaxonomy()->isRegistered( 'category' )
					) {
					...
					if ( $this->isTaxonomyRegistered()
					     && $this->getTaxonomy()->isRegistered( 'tag' )
					) {
					...
					if ( $this->isThumbnail( $post_id ) ) {
						...
						$image_attributes
							...
							'full' );
						$post_data['image_full_src']
							= isset( $image_attributes[0] )
							? $image_attributes[0] : null;
		...
		public function getSingleItem() {
			...
			if ( $this->isMetaBoxRegistered() ) {
				$meta_box_all = $this->getMetaBox()->setPostId( $post_id )
				                     ->getAll();
			...
			if ( $this->isTaxonomyRegistered()
				...
				$post_data['tags'] = $this->getTaxonomy()->setPostId( $post_id )
				                          ->getPostTerms( 'tag' );
			}

			if ( $this->isThumbnail( $post_id ) ) {
				$post_data['image_html'] = \get_the_post_thumbnail( $post_id,
					$this->thumbnail_size );// Thumbnail
			}
		...
		public function isThumbnail() {
			$post_id   = $this->post_id;
			$thumbnail = \in_array( 'thumbnail', $this->supports )
