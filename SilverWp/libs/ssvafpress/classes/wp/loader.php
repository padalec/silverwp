...
class VP_WP_Loader {
	...
	private function __construct() {
		$this->_dependencies = apply_filters( 'vp_dependencies_array',
			VP_Util_Config::instance()->load( 'dependencies' ) );
		$this->_types        = array(
			'option'             => array(),
			'metabox'            => array(),
	...
	public function build() {
		...
		foreach ( $types as $type ) {
			if ( array_key_exists( $type, $rules ) ) {
				$req_scripts = array_merge( $req_scripts,
					$rules[ $type ]['js'] );
				$req_styles  = array_merge( $req_styles,
					$rules[ $type ]['css'] );
			}
		}
		...
		foreach ( $styles as $name => $style ) {
			if ( in_array( $name, $req_styles ) and ! wp_style_is( $name,
					'registered' )
			) {
	...
	private function build_localize_data() {
		...
		$localize        = array(
			...
			// validatable data
			'alphabet_validatable'     => apply_filters( 'vp_alphabet_validatable',
				array( 'vp-textbox', 'vp-textarea' ) ),
			'alphanumeric_validatable' => apply_filters( 'vp_alphanumeric_validatable',
				array( 'vp-textbox', 'vp-textarea' ) ),
			'numeric_validatable'      => apply_filters( 'vp_numeric_validatable',
				array( 'vp-textbox', 'vp-textarea' ) ),
			'email_validatable'        => apply_filters( 'vp_email_validatable',
				array( 'vp-textbox', 'vp-textarea' ) ),
			'url_validatable'          => apply_filters( 'vp_url_validatable',
				array( 'vp-textbox', 'vp-textarea' ) ),
	...
	private function js_unit_register( $name, $extra_deps = null ) {
		...
		if ( isset( $scripts[ $name ] ) ) {
			...
			$is_older   = false;
			$script     = $scripts[ $name ];
			$override   = isset( $script['override'] ) ? $script['override']
				: false;
			if ( $registered ) {
				$is_older = version_compare( $script['ver'],
						$wp_scripts->registered[ $name ]->ver ) == 1;
			}
			if ( ! $registered or ( $is_older and $override ) ) {
				if ( ! is_null( $extra_deps ) ) {
					$script['deps']
						= array_unique( array_merge( $script['deps'],
						$extra_deps ) );
				}
				...
				wp_register_script( $name, $script['path'], $script['deps'],
					$script['ver'], true );
				...
				if ( isset( $script['localize'] ) ) {
					...
					wp_localize_script( $name, $script['localize']['name'],
						$localize );
				}
			}
		}
	...
	private function css_unit_register( $name, $extra_deps = null ) {
		...
		if ( isset( $styles[ $name ] ) ) {
			...
			if ( ! is_null( $extra_deps ) ) {
				$style['deps'] = array_unique( array_merge( $style['deps'],
					$extra_deps ) );
			}
			if ( ! empty( $style['deps'] ) ) {
	...
	public function add_data( $name, $key, $data, $type ) {
		...
		if ( in_array( $key, $array_data ) ) {
			if ( ! isset( $this->{$var_name}[ $name ][ $key ] )
			     || ! is_array( $this->{$var_name}[ $name ][ $key ] )
			) {
	...
	public function add_main_js( $js ) {
		...
		if ( isset( $js['localize'] ) and is_array( $js['localize'] ) ) {
			if ( isset( $js['localize']['name'] ) ) {
				$this->add_js_data( $js_name, 'local_name',
					$js['localize']['name'] );
			}

			if ( isset( $js['localize']['keys'] )
			     and is_array( $js['localize']['keys'] )
			) {
				$this->add_js_data( $js_name, 'local_data',
					$js['localize']['keys'] );
			}
		}
	...
	public function add_types( $types, $key ) {
		$types = (array) $types;
		$this->_types[ $key ]
		       = array_unique( array_merge( $this->_types[ $key ], $types ) );
	}
