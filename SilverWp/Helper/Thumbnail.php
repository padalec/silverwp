...
/*
  Repository path: $HeadURL: https://svn.nq.pl/wordpress/branches/dynamite/igniter/wp-content/themes/igniter/lib/SilverWp/Helper/Thumbnail.php $
  Last committed: $Revision: 2184 $
  Last changed by: $Author: padalec $
  Last changed date: $Date: 2015-01-21 13:20:08 +0100 (Śr, 21 sty 2015) $
  ID: $Id: Thumbnail.php 2184 2015-01-21 12:20:08Z padalec $
 */
 ...
 * @version $Id: Thumbnail.php 2184 2015-01-21 12:20:08Z padalec $
 * @category WordPress
 * @package SilverWp
 * @subpackage Helper
...
class Thumbnail {
    ...
    private function registerCustomImageSize( array $image_sizes ) {
        ...
        foreach ( $image_sizes as $key => $images ) {
            \add_image_size( $key, $images[ 'width' ], $images[ 'height' ], $images[ 'crop' ] );
    ...
    public static function deletePostAttachment( $post_id ) {
        ...
        if ( $post_attachments ) {
            foreach ( $post_attachments as $key => $attachment ) {
                $attachment_info[ $key ] = \wp_delete_attachment( $attachment->ID, true );
            }
        }
    ...
    public static function resizeImage( $image_src, $save_path = null ) {
        ...
        foreach ( $image_size as $s ) {
            ...
            $image  = \wp_get_image_editor( $image_src ); // Return an implementation that extends <tt>WP_Image_Editor</tt>
