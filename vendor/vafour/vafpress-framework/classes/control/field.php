...
abstract class VP_Control_Field implements iFactory {
	...
	protected function _setup_data() {
		...
		// Determine Type
		$type = 'vp-' . strtolower( substr( get_class( $this ),
				strrpos( get_class( $this ), '_' ) + 1 ) );
	...
	protected function _basic_make( $arr ) {
		...
		;
		//add html control attributes
		if ( isset( $arr['html_attributes'] ) ) {
			$this->set_attributes( $arr['html_attributes'] );
		}
	 ...
	 *
	 * @param array $attributes assocjative arrays example:
	 *                          array(
	 *                              'class' => 'class_name'
	 *                          )
	 *
	...
	public function set_attributes( array $attributes ) {
		$this->add_data( 'attributes', $attributes );
	...
	public function add_attribute( $name, $value ) {
		$this->add_single_data( 'attributes', $name, $value );
	...
	public function add_container_extra_classes( $class ) {
		if ( is_array( $class ) ) {
			$this->_container_extra_classes
				= array_merge( $this->_container_extra_classes, $class );
